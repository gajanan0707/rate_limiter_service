...
def set_global_config():
    ...
    try:
        ...
        if (
            "max_global_concurrent_requests" not in data
            or "max_tenant_queue_size" not in data
        ):
            return (
                jsonify(
                    {
...
def set_action_limit(tenant_id, action_type):
    ...
    try:
        ...
        rate_limit_config = RateLimitConfig(
            max_requests=max_requests, window_duration_seconds=window_duration_seconds
        )
...
@config_bp.route(
    "/config/tenant/<tenant_id>/client/<client_id>/action/<action_type>",
    methods=["PUT"],
)
def set_client_limit(tenant_id, client_id, action_type):
    ...
    try:
        ...
        config_manager.set_client_limit(
            tenant_id, client_id, action_type, rate_limit_config
        )
...
def load_config_from_file():
    ...
    try:
        ...
        return (
            jsonify({"message": f"Configuration loaded from {file_path} successfully"}),
            200,
        )
...
def save_config_to_file():
    ...
    try:
        ...
        return (
            jsonify({"message": f"Configuration saved to {file_path} successfully"}),
            200,
        )
